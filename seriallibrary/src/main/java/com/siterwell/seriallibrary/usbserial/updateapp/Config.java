...
import com.siterwell.seriallibrary.usbserial.util.BaseHttpUtil;
import com.siterwell.seriallibrary.usbserial.util.CodeUtil;
...
public class Config {
	 ...
	 * 3.5 获取APP更新信息
	 *
	 * @param updateListenr 回调接口
	 */

	public static void getUpdateInfo(Context mContext, final UpdateListenr updateListenr) {

		BaseHttpUtil.getData(mContext, ApkVerUrl, new AsyncHttpResponseHandler() {
			@Override
			public void onSuccess(int i, Header[] headers, byte[] bytes) {
				updateListenr.onSuccessUpdate(new String(bytes));
			}

			@Override
			public void onFailure(int i, Header[] headers, byte[] bytes, Throwable throwable) {
				updateListenr.onError(CodeUtil.getErrorCode(i, bytes));
			}
		});
	}
